Can't create, edit, or upload … Not enough storage. Get 100 GB of storage for CZK 59.99 CZK 14.99/month for 3 months (personalized price).
#!/usr/bin/python3
...
def eval_atree_rec(node):
    ...
    if node.value == '-':
        return -eval_atree_rec(tree, node.children[0])
        
    if node.value == '*':
        return eval_atree_rec(tree, node.children[0]) * eval_atree_rec(tree, node.children[1])
...
def eval_atree(tree):
    ...
    return eval_atree_rec(tree.root)
...
def const_to_array_rec(node, nums):
    if node is None:
        return
    ...
    for child in node.children:
        nums.append(const_to_array_rec(child, nums))
...
def constants_to_array(tree):
    ...
    nums = []
    constants_to_array_rec(tree.root, nums):
    return nums
...
# nemodifikujte.
def build_atree_rec(node: Node, i: int, array: List[int]):
...
def simplify_atree_rec(node, parent):
    if node is None:
        return
    if len(node.children) == 2:
        if node.children[0] is 0:
            if node.children[0].children is not None:
                parent.children.extend(node.children.children)
